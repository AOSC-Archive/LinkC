...
int wLogin(WINDOW* Console,int Sockfd,LoginData Data){
    ...
    if(ntohs(((MessageHeader*)Package)->StatusCode) == LOGIN_FAILURE)
        return LINKC_FAILURE;
    wLinkC_Debug(Console,"用户密码检验",LINKC_SUCCESS);
    ...
    if(ntohs(((MessageHeader*)Package)->StatusCode) == SET_STATUS_FAILURE)
        return LINKC_FAILURE;
    wLinkC_Debug(Console,"状态设置",LINKC_SUCCESS);
...
int wGetSelfData(WINDOW* Console,int Sockfd,UserData* Data){
    ...
    send(Sockfd,Package,Length,0);
    wLinkC_Debug(Console,"个人数据申请要求发送",LINKC_DONE);
    wLinkC_Debug(Console,"等待服务器回应",LINKC_STARTED);
    if(wTCP_Recv(Console,Sockfd,Buffer,STD_PACKAGE_SIZE,0) == LINKC_FAILURE){
        wLinkC_Debug(Console,"接受数据",LINKC_FAILURE);
        free(Buffer);
        free(Package);
        Buffer = NULL;
        Package= NULL;
    ...
    if(((MessageHeader*)Package)->ActionType != (RETURN_DATA|SELF_DATA)){
        wLinkC_Debug(Console,"服务端返回数据错误",LINKC_WARNING);
        free(Buffer);
        free(Package);
        Buffer = NULL;
        Package= NULL;
    ...
    if(((MessageHeader*)Package)->StatusCode == htons(GET_DATA_FAILURE)){
        wLinkC_Debug(Console,"服务端获取数据",LINKC_FAILURE);
        free(Buffer);
        free(Package);
        Buffer = NULL;
        Package= NULL;
    ...
    wLinkC_Debug(Console,"接收个人数据",LINKC_DONE);
